...
erpnext.accounts.taxes.setup_tax_validations("Transfer Out");
...
erpnext.selling.SalesOrderController = class SalesOrderController extends erpnext.selling.SellingController {
			...
			if(doc.status == 'Closed') {
				if(doc.status !== 'On Hold') {
					...
					if (!doc.__onload || !doc.__onload.has_reserved_stock) {
						// Don't show the `Reserve` button if the Proforma Invoice has Picked Items.
	...
	make_purchase_order(){
		...
		if(!pending_items){
			frappe.throw({message: __("Purchase Order already created for all Proforma Invoice items"), title: __("Note")});
		}
