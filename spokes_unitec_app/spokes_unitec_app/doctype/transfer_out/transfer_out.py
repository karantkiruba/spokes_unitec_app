...
# #class TransferOut(Document):
# #       pass
# class TransferOut(SellingController):
#         def __init__(self, *args, **kwargs):
#                 super(TransferOut, self).__init__(*args, **kwargs)

#         def validate(self):
#                 super(TransferOut, self).validate()
...
class TransferOut(SellingController):
	...
	def validate_po(self):
		...
		if self.po_no and self.customer and not self.skip_delivery_note:
			so = frappe.db.sql(
				"select name from `tabTransfer Out` \
			...
			if so and so[0][0]:
				if cint(
					...
					frappe.msgprint(
						_("Warning: Transfer Out {0} already exists against Customer's Purchase Order {1}").format(
					...
					frappe.throw(
						_(
							"Transfer Out {0} already exists against Customer's Purchase Order {1}. To allow multiple Transfer Outs, Enable {2} in {3}"
						).format(
							frappe.bold(so[0][0]),
							frappe.bold(self.po_no),
							frappe.bold(_("'Allow Multiple Transfer Outs Against a Customer's Purchase Order'")),
			...
			if self.delivery_date:
				for d in self.get("items"):
					...
					if getdate(self.transaction_date) > getdate(d.delivery_date):
						frappe.msgprint(
							_("Expected Delivery Date should be after Transfer Out Date"),
							indicator="orange",
	...
	def validate_warehouse(self):
		super(TransferOut, self).validate_warehouse()
	...
	def validate_with_previous_doc(self):
		super(TransferOut, self).validate_with_previous_doc(
	...
	def on_cancel(self):
		...
		super(TransferOut, self).on_cancel()
	...
	def check_credit_limit(self):
		# if bypass credit limit check is set to true (1) at Transfer Out level,
	...
	def check_nextdoc_docstatus(self):
		...
		if linked_invoices:
			...
			frappe.throw(
				_("Sales Invoice {0} must be deleted before cancelling this Transfer Out").format(
					", ".join(linked_invoices)
				)
			)

	def check_modified_date(self):
		mod_db = frappe.db.get_value("Transfer Out", self.name, "modified")
		...
		for item in self.items:
			if item.supplier:
				supplier = frappe.db.get_value(
					"Transfer Out Items", {"parent": self.name, "item_code": item.item_code}, "supplier"
				)
...
def close_or_unclose_sales_orders(names, status):
	if not frappe.has_permission("Transfer Out", "write"):
		frappe.throw(_("Not permitted"), frappe.PermissionError)

	names = json.loads(names)
	for name in names:
		so = frappe.get_doc("Transfer Out", name)
		if so.docstatus == 1:
			if status == "Closed":
...
def make_material_request(source_name, target_doc=None):
	...
	doc = get_mapped_doc(
		...
		source_name,
		{
			"Transfer Out": {"doctype": "Material Request", "validation": {"docstatus": ["=", 1]}},
			"Packed Item": {
...
def make_delivery_note(source_name, target_doc=None, skip_item_mapping=False):
	...
	target_doc = get_mapped_doc("Transfer Out", source_name, mapper, target_doc, set_missing_values)
...
def make_maintenance_schedule(source_name, target_doc=None):
	...
	if not maint_schedule:
		doclist = get_mapped_doc(
			...
			source_name,
			{
				"Transfer Out": {"doctype": "Maintenance Schedule", "validation": {"docstatus": ["=", 1]}},
				"Transfer Out Items": {
...
def make_maintenance_visit(source_name, target_doc=None):
	...
	if not visit:
		doclist = get_mapped_doc(
			...
			source_name,
			{
				"Transfer Out": {"doctype": "Maintenance Visit", "validation": {"docstatus": ["=", 1]}},
				"Transfer Out Items": {
...
def get_events(start, end, filters=None):
	...
	conditions = get_event_conditions("Transfer Out", filters)

	data = frappe.db.sql(
		"""
		select
			distinct `tabTransfer Out`.name, `tabTransfer Out`.customer_name, `tabTransfer Out`.status,
			`tabTransfer Out`.delivery_status, `tabTransfer Out`.billing_status,
			`tabTransfer Out Items`.delivery_date
		from
			`tabTransfer Out`, `tabTransfer Out Items`
		where `tabTransfer Out`.name = `tabTransfer Out Items`.parent
			and `tabTransfer Out`.skip_delivery_note = 0
			and (ifnull(`tabTransfer Out Items`.delivery_date, '0000-00-00')!= '0000-00-00') \
			and (`tabTransfer Out Items`.delivery_date between %(start)s and %(end)s)
			and `tabTransfer Out`.docstatus < 2
			{conditions}
		""".format(
...
def set_delivery_date(items, sales_order):
	delivery_dates = frappe.get_all(
		"Transfer Out Items", filters={"parent": sales_order}, fields=["delivery_date", "item_code"]
	)
...
def make_work_orders(items, sales_order, company, project=None):
	"""Make Work Orders against the given Transfer Out for the given `items`"""
...
def make_inter_company_purchase_order(source_name, target_doc=None):
	...
	return make_inter_company_transaction("Transfer Out", source_name, target_doc)
...
def create_pick_list(source_name, target_doc=None):
	...
	doc = get_mapped_doc(
		...
		source_name,
		{
			"Transfer Out": {"doctype": "Pick List", "validation": {"docstatus": ["=", 1]}},
			"Transfer Out Items": {
...
def update_produced_qty_in_so_item(sales_order, sales_order_item):
	# for multiple work orders against same Transfer Out item
	...
	frappe.db.set_value("Transfer Out Items", sales_order_item, "produced_qty", total_produced_qty)
...
def get_work_order_items(sales_order, for_raw_material_request=0):
	...
	if sales_order:
		so = frappe.get_doc("Transfer Out", sales_order)
